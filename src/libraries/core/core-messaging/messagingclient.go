...
func (m *AmqpClient) ConnectToBroker(connectionString string) {
	if connectionString == "" {
		logrus.Info("Cannot initialize connection to broker, connectionString not set. Have you initialized?")
	}

	var err error
	m.conn, err = amqp.Dial(fmt.Sprintf("%s/", connectionString))
	if err != nil {
		logrus.Info("Failed to connect to AMQP compatible broker at: " + connectionString)
	}
}
...
func (m *AmqpClient) Publish(body []byte, exchangeName string, exchangeType string) (err error) {
	if m.conn == nil {
		logrus.Error("Tried to send message before connection was initialized. Don't do that.")
	}
	ch, err := m.conn.Channel() // Get a channel from the connection

	defer func() {
		if e := ch.Close(); e != nil {
			err = e // maybe log in the future
		}
	}()
