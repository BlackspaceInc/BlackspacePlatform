...
func TestValidJsonBody(t *testing.T) {
	// arrange
	loginReq := api.CreateAccountRequest{
		Username: "testUser",
	...
	// act
	err = DecodeJSONBody(&resp, req, &api.CreateAccountRequest{})
...
func TestDecodeJsonBodyInvalidContentType(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(testname, func(t *testing.T) {
			...
			err := helper.DecodeJSONBody(&resp, req, &api.CreateAccountRequest{})
...
func TestDecodeJsonBodyInvalidDestInterface(t *testing.T) {
	...
	// assert
	var mr *helper.MalformedRequest
	if errors.As(err, &mr) == false || !strings.Contains(mr.Msg, BADLY_FORMATTED_JSON) {
		t.Errorf("got %s, want %s", mr.Msg, BADLY_FORMATTED_JSON)
	}
}
...
func TestDecodeJsonBodyInvalidEmptyRequest(t *testing.T) {
	...
	var mr *helper.MalformedRequest
	errors.As(err, &mr)
	if mr.Status != http.StatusBadRequest || mr.Msg != REQUEST_BODY_MUST_NOT_BE_EMPTY {
		t.Errorf("got status %d, want status %d, got status %s, want status %s", mr.Status,
			http.StatusBadRequest, mr.Msg, REQUEST_BODY_MUST_NOT_BE_EMPTY)
	}
}
