...
# ENV VARS
#
#   $OVERWRITE      overwrite Blackspace binary even if it already exists (default "true")
...
ARCHIVE_DIR_NAME="Blackspace-$TARGET"
ARCHIVE_DIR="$TARGET_DIR/$ARCHIVE_DIR_NAME"
ARCHIVE_NAME="Blackspace-$TARGET.$ARCHIVE_TYPE"
...
echo "Packaging the Blackspace archive"
...
# Copy the Blackspace binary to /bin

mkdir -p "$ARCHIVE_DIR/bin"
cp -av "$TARGET_DIR/release/Blackspace" "$ARCHIVE_DIR/bin"
...
if [[ $TARGET == *linux* ]]; then
  ...
  cp -av distribution/systemd/Blackspace.service "$ARCHIVE_DIR/etc/systemd"
  mkdir -p "$ARCHIVE_DIR/etc/init.d"
  cp -av distribution/init.d/Blackspace "$ARCHIVE_DIR/etc/init.d"
fi
...
(
  ...
  if [ "$ARCHIVE_TYPE" == "tar.gz" ]; then
    ...
    powershell '$progressPreference = "silentlyContinue"; Compress-Archive -DestinationPath Blackspace-'"$TARGET"'.'"$ARCHIVE_TYPE"' -Path "./'"$ARCHIVE_DIR_NAME"'/*"'
  else
