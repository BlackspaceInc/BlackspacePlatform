...
# SUMMARY
#
#   Builds the Blackspace docker images and optionally
...
build() {
  ...
  if [ -n "$PLATFORM" ]; then
    ...
    docker buildx rm Blackspace-builder || true
    docker buildx create --use --name Blackspace-builder
