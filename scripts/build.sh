...
# ENV VARS
#
#   $OVERWRITE      overwrite Blackspacepace binary even if it already exists (default "true")
#   $CHANNEL        the release channel for the build, "nighly" or "stable" (default `scripts/release-channel.sh`)
#   $FEATURES       a list of Blackspacepace features to include when building (default "default")
...
BINARY_PATH="$TARGET_DIR/release/Blackspacepace"
...
if [ -f "$BINARY_PATH" ] && [ "$OVERWRITE" == "false" ]; then
  echo "Blackspacepace binary already exists at:"
  echo ""
  echo "    $BINARY_PATH"
  echo ""
...
echo "Building Blackspacepace binary"
