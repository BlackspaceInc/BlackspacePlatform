...
PROJECT_ROOT="$(pwd)"
ARCHIVE_NAME="Blackspace-$TARGET.tar.gz"
...
cp -av "$ARCHIVE_PATH" "$RPMBUILD_DIR/SOURCES/Blackspace-$ARCH.tar.gz"
...
rpmbuild \
  ...
  --define "_arch $ARCH" \
  -ba distribution/rpm/Blackspace.spec
...
mv -v "$RPMBUILD_DIR/RPMS/$ARCH/Blackspace-$CLEANED_VERSION-$RELEASE.$ARCH.rpm" "target/artifacts/Blackspace-$ARCH.rpm"
