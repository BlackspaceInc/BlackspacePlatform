...
#   and release Blackspace. This is primarily used in CI.

# Builds a generic Docker image with a `Blackspace-` prefix. The name
# maps to the contained folder.
build_image() {
  local TAG="$1"

  docker build \
    -t "timberiodev/Blackspace-$TAG:latest" \
    -f "scripts/ci-docker-images/$TAG/Dockerfile" \
    .

  docker push "timberiodev/Blackspace-$TAG:latest"
}
