...
# After we build Blackspace docker image, instead of pushing to the remote repo,
...
if [[ -z "${CONTAINER_IMAGE:-}" ]]; then
  ...
  if is_minikube_cache_enabled; then
    ...
    # tests.
    CONTAINER_IMAGE_REPO="${CONTAINER_IMAGE_REPO:-"localhost/Blackspace-test"}"
  else
  ...
  if [[ "${QUICK_BUILD:-"false"}" == "true" ]]; then
    ...
    # Build docker image with Blackspace - the same way it's done for releses. Don't
